...
public class two_player_two_device_canvas extends View { //you have to create a new java file and then insert the same file in the xml of the page in which you want the canvas
    ...
    public two_player_two_device_canvas(Context context, AttributeSet attrs) {
        super(context, attrs);
        ctx = context;
        ...
        bluetoothDevice = two_device_two_player_name.mBluetoothDevice;
        bluetoothSocket = two_device_two_player_name.mBluetoothSocket;
            ...
            if (touchX < canvasSide && touchX > 0 && touchY < canvasSide && touchX > 0) {
                ...
                if (a[row][col] == 0) {
                    ...
                    if (connectedThread != null) {
                        ...
                        if (turn == 1) {
                            two_player_names.p1Name = two_device_two_player_name.MyName.trim().toUpperCase();
                            two_player_names.p2Name = two_device_two_player_name.OpponentName.trim().toUpperCase();
                            ...
                            two_player_names.p2Name = two_device_two_player_name.MyName.trim().toUpperCase();
                            two_player_names.p1Name = two_device_two_player_name.OpponentName.trim().toUpperCase();
                            String tmp = dbh.checkUser(two_player_names.p1Name);
                            if (tmp.equals("FOUND")) {
                                //Toast.makeText(two_player_names.this,"found 1",Toast.LENGTH_SHORT).show();
                            } else {
                    ...
                    if (!oncewin && !oncedrawen) {
                        ...
                        two_device_two_player.act_2p.setResult(3, intent);
                        two_device_two_player.act_2p.finish();
                    }
                }
            }
        }
    ...
    public void showAlert(final String str) {
        ...
        two_device_two_player.act_2p.runOnUiThread(
                new Runnable() {
                    @Override
                    public void run() {
    ...
    public class ConnectedThread extends Thread {
        ...
        public void run() {
            ...
            if (cnt == 0) {
                try {
                    byte[] ByteArray = two_device_two_player_name.MyName.getBytes();
                    connectedThread.write(ByteArray);
                    cnt++;
                } catch (Exception e) {
                    ...
                    if (readMessage.contains(";")) {
                        ...
                        two_device_two_player.act_2p.runOnUiThread(
                                new Runnable() {
                                    @Override
                                    public void run() {
                                        Toast.makeText(two_device_two_player.act_2p, two_device_two_player_name.MyName + " vs " + two_device_two_player_name.OpponentName, Toast.LENGTH_SHORT).show();
                                    }
                                }
                        );
                        ...
                        try {
                            Log.i(TAG, "Hello");
                            two_device_two_player_name.OpponentName = readMessage;
                            Log.i(TAG, two_device_two_player_name.MyName + " vs " + two_device_two_player_name.OpponentName);
                            two_device_two_player.act_2p.runOnUiThread(
                                    new Runnable() {
                                        @Override
                                        public void run() {
