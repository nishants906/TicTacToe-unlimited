...
/**
 * Created by Nishant on 03/08/17.
 */
...
public class Gameu5 {
    ...
    private char[][] cells;
    private char[][] mBoard = ((char[][]) Array.newInstance(Character.TYPE, new int[]{this.BOARD_SIZE, this.BOARD_SIZE}));
    ...
    public Gameu5() {
        ...
        this.Val = (int[][]) Array.newInstance(Integer.TYPE, new int[]{this.n, this.n});
        this.cells = (char[][]) Array.newInstance(Character.TYPE, new int[]{this.n, this.n});
    }
    ...
    private State GetMaxNode() {
        ...
        for (i = 0; i < list.size(); i++) {
            this.Val[((State) list.get(i)).p.x][((State) list.get(i)).p.y] = 0;
        }
        return (State) list.get(this.rand.nextInt(list.size()));
    }
    ...
    public int[] Solve(char[][] bb, char Player) {
        ...
        EvalueCaroBoard(this.cells, PTWO);
        ArrayList<State> list = new ArrayList();
        for (i = 0; i < this._branch; i++) {
            list.add(GetMaxNode());
            if (((State) list.get(i)).val > 1538) {
                break;
            }
        }
        ...
        for (i = 0; i < list.size(); i++) {
            this.cells[((State) list.get(i)).p.x][((State) list.get(i)).p.y] = PTWO;
            int t = MinVal(this.cells, (State) list.get(i), -this.INT_MAX, this.INT_MAX, 0);
            if (maxp < t) {
                maxp = t;
                ListChoose.clear();
                ListChoose.add((State) list.get(i));
            } else if (maxp == t) {
                ListChoose.add((State) list.get(i));
            }
            this.cells[((State) list.get(i)).p.x][((State) list.get(i)).p.y] = EMPTY_SPACE;
        }
        int x = this.rand.nextInt(ListChoose.size());
        result[0] = ((State) ListChoose.get(x)).p.x;
        result[1] = ((State) ListChoose.get(x)).p.y;
        return result;
    }
    ...
    private int MaxVal(char[][] b, State s, int alpha, int beta, int depth) {
        ...
        for (i = 0; i < list.size(); i++) {
            b[((State) list.get(i)).p.x][((State) list.get(i)).p.y] = '0';
            alpha = Math.max(alpha, MinVal(b, (State) list.get(i), alpha, beta, depth + 1));
            b[((State) list.get(i)).p.x][((State) list.get(i)).p.y] = EMPTY_SPACE;
            if (alpha > beta) {
                break;
            }
        }
    ...
    private int MinVal(char[][] b, State s, int alpha, int beta, int depth) {
        ...
        for (i = 0; i < list.size(); i++) {
            b[((State) list.get(i)).p.x][((State) list.get(i)).p.y] = 'X';
            beta = Math.min(beta, MaxVal(b, (State) list.get(i), alpha, beta, depth + 1));
            b[((State) list.get(i)).p.x][((State) list.get(i)).p.y] = EMPTY_SPACE;
            if (alpha >= beta) {
                break;
            }
